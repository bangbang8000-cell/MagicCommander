...
采用“项目中心 + 模板中心 + 工作流面板，分层重组”方案。

### 2.1 方案边界

保留现有左侧 ActivityBar、中栏 Sidebar、编辑区、底部面板结构，不新增独立 Dashboard 首页，不引入数据库或新的项目索引文件。第一轮只在现有侧边栏体系内重组项目管理、模板管理和工作台流程。现有 `listExamples`、`saveAsExample`、从 template 创建项目等“示例项目”能力将被产品化为模板中心能力。
...
3. 增加项目状态徽标的轻量计算与展示。
4. 扩展模板元信息模型和 IPC：列表、读取、编辑、删除、另存为模板。
5. 实现模板中心展示：搜索、排序、卡片/列表、详情、编辑弹窗。
6. 打通从模板创建项目和项目另存为模板的闭环。
7. 重组 `WorkbenchPanel` 为流程化卡片，并加入模板快捷动作入口。
8. 统一工作台执行范围计算逻辑。
9. 增加结果入口和错误反馈区域。
10. 执行类型检查和构建验证。
...
Phase 1-A 第一轮完成后，MagicCommander 应具备更清晰的多项目操作能力、模板资产复用能力和工作台流程闭环。项目可以沉淀为模板，模板可以快速创建新项目，工作台可以围绕单项目或批量项目完成检查、执行和查看结果。这将为后续 Phase 1-B 的渲染准确率检查、模板变量校验、输出历史对比、模板资产质量评级，以及 Phase 2 AI Hub 的智能建议入口打基础。
