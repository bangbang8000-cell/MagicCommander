# MagicCommander 更新日志

本文件为发布说明的单一事实来源（`npm run sync-version -- --release-notes` 自动抽取当前版本段）。

## [5.1.0] - 2026-09-07

### 新增

- **Agent Connect（MCP Server，5.1.1–5.1.10 全量）**：以标准 MCP 协议接入外部 AI Agent / 编程智能体（Claude / Codex / Trae Work / VS Code 等），直接查询/创建/更新/渲染项目、模板、设备库与输出。
  - **双场景模型**：编译态（产品使用，不修改软件本体）/ 源码态（开发者，CLI 透传 + 沙箱文件系统）；
  - **确定性语义层**：入参契约（L1 JSON Schema）/ 语义校验（L2）/ 幂等事务（L3）；
  - **异步任务**：task_id + 进度轮询 / 等待 / 取消；**操作审计**（脱敏）；**远程模式试点**（TLS + 强鉴权 + 按域授权）；**Agent 反馈自优化**闭环。
- **MCP 接入指南（5.1 收尾）**：帮助菜单新增「MCP 接入指南」（工作区加载 + 一键复制接入配置）；设置页 Agent Connect 提供「复制接入配置 / 打开指南」按钮；用户指南同步更新至 Agent Connect 完整版。

### 改进

- 工程修复：ci.yml 移除无效步骤属性（workflow 可正常触发）；用户指南补充 MCP 接入指南入口。

## [5.0.10] - 2026-09-06

### 新增

- 5.0 系列第 10 版：内容资产升级与收官（示例库扩充 + 内容库升级 + 5.0 系列验收）。
- 版本发布对齐：双端示例库扩充至 7 个（H100-256台/512台-RoCE、国产-昇腾-256 规模化与多厂商场景）、AL 模板 25 / 设备 126 全量重测通过、官网 5.0 系列发布收官。

## [5.0.9] - 2026-09-06

### 新增

- 5.0 系列第 9 版：交付与运维强化（升级体验可靠化 + 企业部署基座 + 平台可观测）。
- 升级体验（509-a）：安装包下载**断点续传**（`.part` 记录偏移，失败续传）+ **SHA-512 强校验**（读 latest.yml sha512，缺失退化 Content-Length 比对）+ **版本回滚**（安装前留存上一版本安装包，启动异常可回退）+ **灰度通道**（stable/beta，按通道选择 latest\*-channel.yml）。

### 改进

- 企业部署基座（509-b）：新增 `enableEnterpriseDeploy` 配置开关（默认关，隐藏）；开启后支持**内网 updateUrl 镜像**、**代理下载**（系统代理 / 自定义代理 CONNECT 隧道）、**版本锁定**（平台下发 min_required_version/allowed 校验）。
- 平台可观测 + 版本分发（509-c）：health/full 增磁盘水位/进程探测；多通道 versions.json 下发 sha512 与镜像；备份统一到 auto_backup.sh；告警脚本 check_alerts.sh + 日志轮转。
- 云交付基座（509-e）：分享预览页在 `CLOUD_DELIVERY_ENABLED` 开启时提供「下载方案包」分发下载（默认关闭）。

## [5.0.8] - 2026-09-05

### 新增

- 5.0 系列第 8 版：质量与可靠性深化。
- 覆盖率棘轮上移：前端 lines ≥36%、后端 ≥64%（scripts/check_coverage_baseline.py 只升不降门禁），并产出覆盖率攻坚报告。
- 补齐高价值纯逻辑单测：icons / escapeHtml / log.store / render.store / errorService 等，前端覆盖率显著上移。

### 改进

- 设备库联网校对（508-d）：S9827、S9825-64D、RG-S6980-64QC、S9820-8C 等关键 400G 形态对齐（OSFP → QSFP-DD/QSFP112），NVIDIA MQM9700/9790 形态修正为 OSFP；CloudEngine 9860/8860 机架高度与端口数修正；双端校验脚本全部通过。

### 修复

- CI e2e 加固：install-deps / Playwright 依赖 / E2E 步骤增加 `retry-on` + `timeout-minutes`，降低环境性 red。

### 安全

- 平台依赖迁移：`python-jose` → `PyJWT`，消除 ecdsa 漏洞（PYSEC-2026-1325）；pip-audit 高危清零。

## [5.0.7] - 2026-09-05

### 新增

- 5.0 系列第 7 版：性能与效率优化。
- 性能基准对比基线（scripts/bench_perf.py --json/--compare）：规范化 JSON 基准报告 + 与上一版基线逐场景 delta/PASS/REGRESS 对比；新增 512 项目批量 dry-run 渲染场景（≈5120 台设备，量化规模化渲染成本）。

### 改进

- 批量渲染并发提升与内存守卫（batch.store）：默认并发 2→3，并按设备可用内存动态收敛（<4GB→1、<8GB→2、其余→3），避免大项目批量渲染 OOM；批量任务前清理既有 bench 中间目录，保证基准测量干净自洽。
- Monaco 编辑器懒加载：移除 main.tsx 顶层导入，仅当打开编辑器时才按需加载本地打包的 monaco-editor，降低首屏 bundle 与启动内存。
- Word 预览（docx-preview）动态导入（既有实现保持）。

### 基准（5.0.7 本地实测，阈值≤实测 PASS）

- 批量渲染 100 项目（≈1000 台）：≤90s / 实测 31s
- 单项目全量渲染：≤30s / 实测 0.57s
- 万行参数表数据准备（10k）：≤30s / 实测 1.14s
- 批量渲染 512 项目（≈5120 台）：≤240s / 实测 174s

## [5.0.6] - 2026-09-05

### 新增

- 5.0 系列第 6 版：3D 可视化。
- 机房/机柜 3D 只读视图（AIDC 导入）：由 AIDC 计划 deviceList 派生机柜网格与设备 U 位（42U），WebGL(react-three-fiber) 机房全景 + SVG 等距单柜视图，OrbitControls 轨道/缩放/旋转。
- 热力图：机柜按功率使用率着色（绿→黄→红，powerToHeatColor 与 AutoLink 对齐），带图例。
- 3D 导出：机房 WebGL PNG 导出 + 机柜 SVG/PNG 导出。
- 2D↔3D 一致性：点击机房机柜联动展示对应柜的 SVG 等距视图。

### 改进

- AIDC 导入结果新增“3D 视图”页签（tables/verify/topo/changelog 之外）。

## [5.0.5] - 2026-09-04

### 新增

- 5.0 系列第 5 版：文档与知识。
- 文档工作台：一级导航 doc 入口 + DocPanel 三页签（文档：评审包/评审 PDF/Markdown 生成导出与产物列表；指南：用户指南查看；知识库）。
- 知识库（KnowledgeEngine）：knowledge/*.md + 伴生 meta 持久化，中文 2-gram/英文分词加权检索式召回 Top-K（默认 5），分类/项目过滤；工具 list_knowledge/search_knowledge/add_knowledge；/api/chat/knowledge* 端点 + 前端知识库面板；与技能（操作指引）解耦。
- AI 上下文管理：系统提示词组装新增知识库上下文注入（按项目+会话关键词动态检索 Top-K）；ChatRequest.knowledge/knowledge_ids 全链路透传；知识变更刷新 system prompt 缓存；设置页知识库注入开关。

### 改进

- 一级导航新增 doc（文档工作台）入口（Ctrl+Shift+D）。

## [5.0.4] - 2026-09-04

### 新增

- 5.0 系列第 4 版：协作与生态。
- 协作分享：只读快照（mc.share-snapshot/1，≤2MB）生成 + 云中心分享链接（创建/列表/撤销）+ 平台免登录预览页。
- 模板市场生态：评分（1-5 upsert/均值/人数/我的分）+ 订阅（订阅/取消）+ featured 精选徽标 + 列表注入 rating_avg/rating_count/is_subscribed + 客户端展示（星标/订阅按钮/精选徽标）。
- 设备库云同步：拉取平台设备库合并本地 + 发布本地库（扁平数组/bundle 格式兼容）+ 云端数/本地数/上次同步状态。

### 改进

- 平台生态服务化（COLLAB_ENABLED/MARKET_ENABLED 开关）。

## [5.0.3] - 2026-09-03

### 新增

- 5.0 系列第 3 版：AI Agent 工作流深化。
- 多步自主任务编排（Plan→Execute→Verify 状态机）：full_auto 自主全流程 / advisor 计划级+每步确认 / semi_auto 计划级+关键步确认；接线 accuracy.verify_tool_result 一致性校验；会话保留任务上下文（task_id/plan/step/verify）。
- 技能自学习闭环：反馈持久化（成功/失败/成功率/最近样本）+ 阈值自动修订技能定义（自学习改进记录）+ skill_optimize 工具；run_stream 接线 record_usage/反馈采集；技能传输 manifest v2（技能级元数据，兼容旧包）。
- MCP 工具接入（协议层，非 Agent 框架）：MCP server 配置/stdio 生命周期/工具发现 → 动态注册（mcp: 命名空间防冲突）/ 执行分发；双引擎（自有/Hermes）共享；前端 MCP 管理区。

### 改进

- 前端 chat 任务进度卡片（plan→step→verify 徽标）+ 步骤级确认。

## [5.0.2] - 2026-09-03

### 新增

- 5.0 系列第 2 版：AI Agent 底座（Hermes 并存）。
- 统一 AgentProvider 抽象（会话/工具/技能/记忆四域接口）：OwnAgentProvider（自有引擎适配）+ HermesAgentProvider（适配 NousResearch/hermes-agent，探测运行时是否安装）。
- 「AI 引擎」配置三选一（自有=默认 / Hermes / 自动）：配置持久化、一键切换不丢会话（会话按引擎命名空间隔离）；auto 模式 Hermes 可用则用 Hermes 否则回退自有。
- Hermes 未安装友好提示：chat 返回安装指引（pip install hermes-agent + 官网），前端渲染提示卡片。
- 审计修订：hermes 经 AgentProvider 并存（其余未接入平台 harness/langchain 等仍拦截）。

### 改进

- 双端 AI 引擎路由（aihub:getEngine/setEngine + SettingsPanel 引擎下拉）。

## [5.0.1] - 2026-09-03

### 新增

- 5.0 系列第 1 版：内容与渲染准确性攻坚（三位一体：客户端 + 平台 + 官网）。
- 四示例复核修正：IB 示例参数网/存储网型号对齐 NVIDIA Quantum QM9700（消除「IB 组网用 RoCE 交换机」协议不一致）；example2 补齐 README。
- 模板库门禁强化：新增参数合理性/协议兼容性检查（PFC/CNP 队列、收敛比、AS/VLAN/IP 段、端口速率与设备库匹配、协议匹配）。
- 设备库扩充：新增 NVIDIA Quantum QM9700 等 4 条 IB 型号 + fabric 感知角色解析；新增设备库校验脚本（id 唯一/字段完整/协议一致性/AL 对账）+ CI。
- 渲染核对矩阵扩展：新增设备数 / 命名规范 / IP 连通 / 连接表 / 收敛比核对（verify_rendered.py verify_structural）。

### 改进

- 渲染 workspace 隔离修复；示例渲染确定性。

## [4.9.0] - 2026-09-03

### 新增

- 4.9 示例资产与收官（4.0 系列收官版）。
- AIDC 四示例项目：64H100-IB / 64H100-RoCE / 128H100-IB / 128H100-RoCE（机房矩阵定稿 + 机柜设备参数全配 + 宏观参数 IB/RoCE 协议正确、收敛比合理；可打开 / 可渲染 / 可导出 / 可导入项目包；基于 AutoLink 规划导入闭环）。
- 示例资产库 / 模板中心整合：4 个 AIDC 示例注册模板中心，可基于示例创建副本（忽略 output/yaml）。
- 示例自动化验收：4 示例 ×（打开 / 渲染 / 导出 / 回灌 / golden）脚本门禁 + CI。

### 改进

- 用户指南 / About 收官：示例项目章节 + 应用内 About 提及示例资产。

## [4.8.0] - 2026-09-02

### 新增

- 4.8 互操作与交付强化。
- 项目包往返强化：导出可移植项目包（manifest + 每文件 sha256 + 身份溯源）+ 按身份导入（同 projectId skip/update/new，保留 allocator_state）。
- 导入导出格式增强：项目历史快照导出为文件 + 导入合并去重；各格式（Excel/JSON/ZIP/快照/报告）可往返。
- 跨端资产互灌：设备库/技能库/模板可移植导入导出（schema 标识，合并去重 + 冲突提示）。
- 交付物清单与校验：渲染批次 manifest.json（逐文件 sha256 + 统计 + render_hash）+ 批次完整性校验。
- 报告/评审强化：项目评审报告/评审包（聚合校验报告 + 命令核对矩阵 + 项目摘要 + 交付清单 → zip/Markdown + PDF）。

### 改进

- 渲染备份测试消除秒级时序偶发失败。

## [4.7.0] - 2026-09-02

### 新增

- 4.7 部署运维与可观测。
- 更新兜底链（对齐 AL）：latest.yml 直查 + 安装包直接下载（Content-Length 完整性校验）+ Releases 页兜底 + 启动自动检查开关（checkUpdateOnStart）。
- 诊断中心：日志 / 审计 / 崩溃 / 性能 / 系统信息一处可查 + 一键导出支持包（zip）。
- 健康检查/自检：环境 / 引擎（AI Hub + Python）/ 网络 / 依赖，可读报告 + 导出 JSON。
- 本地遥测：本地化、脱敏、默认关闭、可配置、可导出（telemetry.jsonl）。

### 改进

- 审计日志：关键操作（项目新建/打开/保存/渲染/导出/删除）写入 audit.jsonl（脱敏）。
- 安装/升级体验：构建配置校验脚本 + 离线/无网络友好提示 + 完整性校验显示。

## [4.6.0] - 2026-09-02

### 新增

- 4.6 质量与测试体系。
- 测试覆盖率门禁：后端 pytest-cov（≥55%，实测 60.67%）+ 前端 vitest coverage；CI 覆盖率 job + 阈值常量单源 + 基线棘轮（只升不降）。
- 测试数据资产：样例项目/参数表/模板/渲染基线（清单 manifest + README，pytest/vitest 双端消费）。

### 改进

- 测试报告：聚合 pytest/vitest/golden/性能/校验门禁 → `reports/quality_report.json` + HTML。
- 质量仪表盘：「质量」标签（覆盖率/门禁/测试通过率/校验通过率/性能基准，本地聚合）。

## [4.5.0] - 2026-09-02

### 新增

- 4.5 数据准确性与校验。
- 一致性校验引擎（参数表完整性 / 模板与参数一致性 / 配置字段校验）。
- 导出数据核对（渲染产物与参数/模板一致性 + 漂移检测）。

### 改进

- IP 规划校验（掩码 / 子网重叠 / 网关冲突 / 越界 / 重复）。
- AI 规划器准确性校验（工具返回与实际状态核对）。
- 校验面板（一键校验 / 按严重度分组 / 定位 / 导出报告 JSON / 门禁提示）+ CI 校验门禁。

## [4.4.0] - 2026-09-01

### 新增

- 4.4 效率与自动化工作流。
- 统一快捷键规范（10 标准键：撤销/重做、命令面板、侧边栏、终端、新建、渲染、导出、项目面板、设置、Cheatsheet）+ Cheatsheet。
- 批量操作增强：批量渲染（并行/进度/失败汇总）、批量导出（多项目包）、批量编辑（参数批量应用）。

### 改进

- 一键管线「导入规划 → 渲染 → 导出」（步骤状态/可中断/重试）+ 模板批处理。
- 最近使用/收藏/模板入口（欢迎页「最近项目」「从模板新建」）。
- 命令面板命令全集（项目/模板/渲染/导出/批量/管线/最近收藏/设置/终端）。

## [4.3.0] - 2026-09-01

### 新增

- 4.3 AI 智能体验深化（不含 Hermes）。
- AI 项目/模板操作工具：list / create / update / delete / import / export / create_from_template / preview（8 工具，权限分级 + 校验 + 可读错误）。
- 批量 AI 操作命令化（终端 export/template/batch + 命令面板批量渲染/导出/项目/模板）。

### 改进

- 对话深化：摘要衔接「摘要后继续」、会话归档/取消归档、确认卡片可编辑工具参数（b64）。
- 技能/记忆深化：技能库 list/详情/启用禁用 + 持久化；记忆去抖/prompt 缓存失效测试锁定。
- AI 能力矩阵标准化（8 维度：工具/对话/流式/权限/自主/技能/记忆/规划器）+ 无 Hermes 审计。

## [4.2.0] - 2026-09-01

### 新增

- 4.2 稳定性与数据安全。
- 崩溃上报：本地转储 + 脱敏 errors.log（修复 Bearer/JWT 脱敏顺序）+ 渲染崩溃自动 reload + 未捕获异常兜底。
- 项目编辑自动保存（防抖 800ms + 关闭前 flush）+ 崩溃/关闭后恢复。

### 改进

- 编辑撤销命令栈（深拷贝快照 / 栈深 50 / Ctrl+Z+Y + 工具条，重启保留）。
- 项目版本回滚（`.mc_history/` 最近 10 版）+ 备份轮转（`.mc_backups/` 5 份）+ 一键恢复。
- 大项目性能：Web Worker 化大参数表数据准备 + 万行参数表虚拟化渲染 + 性能门禁场景 C（万行 ≤30s）。
- 性能仪表盘（操作耗时 / 内存 / 渲染长任务 / 基准对比）。

## [4.1.0] - 2026-09-01

### 新增

- 4.1 视觉与品牌统一。
- 主题系统 DTCG 化：light/dark/system/**high-contrast** 四主题，无闪变 + 持久化。
- 高对比主题（WCAG AA）：正文 21:1、次要 14.7:1、弱化 7.6:1、边界 4.8:1、主按钮 8.7:1。

### 改进

- 组件库视觉收敛到契约 token（Button/Input/Select/Modal/Table/Tabs/Popover/Toast/ContextMenu）。
- 品牌资产统一（启动画面/About/徽标/字体栈按契约）。
- 空态/加载/错误态统一（EmptyState/LoadingState/ErrorState）。

## [4.0.0] - 2026-08-29

### 新增

- 4.0 系列启动 · 工程基座与质量门禁。
- 引入 Playwright Electron E2E（冒烟三件套：启动→建项目→渲染→导出）。
- 新增 golden 基线、模板校验、性能门禁、渲染安全基线四道 CI 门禁（对齐 AL 工程化）。
- 版本单源 + 发布说明自动抽取（`sync-version --release-notes`）。

### 改进

- 设计 token 单源扩展（按双端 4.0 契约：语义色/中性/surface/圆角/阴影/间距/动效/字体）+ 断言单测防漂移。
- 组件行为契约（Modal/Popover/ContextMenu/Select/Toast）补齐（ESC 关闭/焦点管理/受控）。
- 工程对齐：ESLint `no-unused-vars` 升 error，lint 0 error 门禁。

## [3.10.7] - 2026-08-29

### 新增

- 会话工具栏上下文摘要 + 长会话截断提示（上下文压缩）。
- 打磨轮（4.0 前最终打磨）用户体验增强。

### 改进

- 长对话上下文自动压缩，保持 AI 会话稳定与响应质量。
- 项目/模板/渲染链路稳定性优化。

### 修复

- 修复多处 3.10 系列打磨中发现的问题（详见各分项提交说明）。

### 质量门禁

- 引入 4.0.0 工程基座前置：E2E（Playwright Electron）、golden 基线、模板校验、性能门禁、渲染安全基线、版本/发布说明单源（对齐 AL）。
