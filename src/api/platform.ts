...
export const auth = {
  ...
  pollStatus: (sessionId: string) => request<ScanStatus>(`/api/v1/auth/scan/status/${sessionId}`),

  refresh: () => request<{ token: string }>('/api/v1/auth/token/refresh', { method: 'POST' }),

  health: () => request<AuthHealth>('/api/v1/auth/health'),
}
...
export const projects = {
  list: () => request<{ projects: RemoteProject[] }>('/api/v1/projects'),
  ...
  delete: (owner: string, repo: string) => request<void>(`/api/v1/projects/${owner}/${repo}`, { method: 'DELETE' }),
...
export const client = {
  ...
  version: () => request<VersionInfo>('/api/v1/client/version'),

  notifications: () => request<{ announcements: Announcement[] }>('/api/v1/client/notifications'),
...
export const templates = {
  list: (q?: string, category?: string, page?: number, limit?: number, sort?: string) => {
    ...
    const qs = params.toString()
    return request<{ templates: RemoteTemplate[]; total: number; page: number; limit: number }>(
      `/api/v1/templates${qs ? '?' + qs : ''}`,
    )
  },

  detail: (owner: string, repo: string) => request<RemoteTemplate>(`/api/v1/templates/${owner}/${repo}`),

  downloadUrl: (owner: string, repo: string) => `${_baseUrl}/api/v1/templates/${owner}/${repo}/download`,
  ...
  mine: () => request<{ templates: RemoteTemplate[] }>('/api/v1/templates/mine'),
...
export const user = {
  ...
  giteaCredentials: () =>
    request<{ username: string; password: string; gitea_url: string }>('/api/v1/user/gitea-credentials', {
      method: 'POST',
    }),
}
