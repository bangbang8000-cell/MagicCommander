...
export function ChatPanel() {
  ...
  useEffect(() => {
    const poll = async () => {
      try {
        ...
        setAIHubError((e instanceof Error ? e.message : String(e)) || t('chat:aihub.error.hubFailed'))
      }
    }
    poll()
    const i = setInterval(poll, 10000)
    return () => clearInterval(i)
  }, [setAIHubStatus, setAIHubProviders, t])
  ...
  useEffect(() => {
    ;(async () => {
      try {
        ...
        if (!s.running && !s.installing) {
          ...
          for (let i = 0; i < 15; i++) {
            ...
            if (st.running) {
              setAIHubStatus(st)
              setAIHubError(null)
              setAIHubProviders(await window.electron.aihub.getProviders())
              return
            }
          }
        ...
        console.error('[ChatPanel] AI Hub', e)
        setAIHubError((e instanceof Error ? e.message : String(e)) || t('chat:aihub.error.hubFailed'))
      }
    })()
  }, [setAIHubStatus, setAIHubProviders, t])
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      setLastError((e instanceof Error ? e.message : String(e)) || t('chat:aihub.error.unknown'))
    }
  ...
  const visibleSessions =
    overflowCount > 0 ? sortedSessions.slice(0, sortedSessions.length - overflowCount) : sortedSessions
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div
        className={clsx(
          'flex items-center gap-1.5 px-3 py-1.5 border-b shrink-0',
          isDark ? 'border-gray-700 bg-gray-900/60' : 'border-gray-200 bg-gray-50',
        )}
      >
        <span
          className={clsx(
            'flex items-center gap-1 text-[11px] px-1.5 py-0.5 rounded-full shrink-0',
          ...
          title={
            aiHubStatus.running
              ...
              : aiHubStatus.lastError || t('chat:aihub.status.stopped')
          }
        >
          {aiHubStatus.installing ? (
        ...
        <select
          ...
          className={clsx(
            'text-[11px] px-1.5 py-0.5 rounded border outline-none shrink-0 max-w-[130px]',
      ...
      <div
        ref={tabBarRef}
        className={clsx(
          'flex items-center gap-0.5 px-1.5 py-1 border-b shrink-0 overflow-hidden',
      ...
      >
        ...
        {overflowCount > 0 && (
          <div className="relative shrink-0" ref={historyRef}>
            ...
            {historyOpen && (
              <div
                className={clsx(
                  'absolute top-full right-0 z-50 mt-1 w-56 py-1 rounded-lg shadow-lg border',
                    ...
                    >
                      ...
                      <button
                        ...
                        className={clsx(
                          ...
                          isDark ? 'hover:bg-red-900/50 text-gray-500' : 'hover:bg-red-100 text-gray-400',
                        )}
                      >
                        <Trash2 size={10} />
                      </button>
        ...
        <button
          ...
          className={clsx(
            'flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[11px] transition-colors shrink-0',
        ...
        {!activeSession || activeSession.messages.length === 0 ? (
          ...
          <div className="py-1">
            {activeSession.messages.map((msg) => (
              <ChatMessageBubble key={msg.id} message={msg} isDark={isDark} fontSizeClass={fontSizeClass} />
            ))}
      ...
      {aiHubError && (
        ...
        >
          <div className="flex items-center gap-1.5">
            <AlertCircle size={12} />
            <span>
              {t('chat:aihub.error.serviceNotRunning')}: {aiHubError}
            </span>
          </div>
          <button
            onClick={async () => {
              try {
                setAIHubError(null)
                await window.electron.aihub.start()
                setAIHubStatus(await window.electron.aihub.status())
              } catch (e) {
                setAIHubError((e instanceof Error ? e.message : String(e)) || t('chat:aihub.error.hubFailed'))
              }
            }}
