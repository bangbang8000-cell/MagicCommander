  ...
  return (
    <div className="flex flex-col h-full overflow-auto p-3 space-y-3">
      <div className="space-y-1.5">
        <h4 className={clsx('text-xs font-semibold', isDark ? 'text-gray-200' : 'text-gray-700')}>{t('label.selectProject')}</h4>
        <select
        ...
        >
          <option value="">{t('label.selectProjectPlaceholder')}</option>
          {projects.map((p) => (
      ...
      {showConfig && (
        <div className={clsx('border rounded p-2 space-y-2', isDark ? 'bg-gray-800/60 border-gray-700' : 'bg-gray-50 border-gray-200')}>
          <h5 className={clsx('text-[10px] font-semibold', isDark ? 'text-gray-200' : 'text-gray-700')}>{t('label.printConfig')}</h5>
          <div className="space-y-1.5 text-[10px]">
            <div className="flex items-center gap-1">
              <label className={clsx('w-14 shrink-0', isDark ? 'text-gray-400' : 'text-gray-600')}>{t('label.paper')}</label>
            ...
            <div className="flex items-center gap-1">
              <label className={clsx('w-14 shrink-0', isDark ? 'text-gray-400' : 'text-gray-600')}>{t('label.orientation')}</label>
            ...
            <div className="flex items-center gap-1">
              <label className={clsx('w-14 shrink-0', isDark ? 'text-gray-400' : 'text-gray-600')}>{t('label.labelsPerPage')}</label>
      ...
      {isLabelPrinting && (
        <div className="space-y-1">
          <div className={clsx('flex justify-between text-[10px]', isDark ? 'text-gray-300' : 'text-gray-600')}>
            <span>{currentMessage || t('label.printing')}</span>
            <span>{progress}%</span>
          </div>
