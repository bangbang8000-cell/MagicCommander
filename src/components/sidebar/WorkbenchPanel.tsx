...
export const WorkbenchPanel = React.memo(function WorkbenchPanel() {
  ...
  const openInFolder = async () => {
    ...
    try {
      ...
      showSuccess('已在文件管理器中打开')
    } catch (err) {
      showError(`打开失败: ${(err as Error).message}`)
    }
  }
  ...
  return (
    <div className={clsx('flex flex-col h-full overflow-auto', isDark ? 'bg-gray-900' : 'bg-white')}>
      <div className="p-3 space-y-3">
              ...
              {projectInfo && (
                <>
                  ...
                  <div className="flex flex-wrap gap-1 pt-1">
                    {projectInfo.structure?.excel && <span className={clsx('px-1.5 py-0.5 rounded text-[10px]', isDark ? 'bg-green-900/40 text-green-300' : 'bg-green-100 text-green-700')}>Excel</span>}
                    {projectInfo.structure?.templates && <span className={clsx('px-1.5 py-0.5 rounded text-[10px]', isDark ? 'bg-blue-900/40 text-blue-300' : 'bg-blue-100 text-blue-700')}>模板</span>}
                    {projectInfo.structure?.para && <span className={clsx('px-1.5 py-0.5 rounded text-[10px]', isDark ? 'bg-purple-900/40 text-purple-300' : 'bg-purple-100 text-purple-700')}>参数</span>}
                    {projectInfo.structure?.output && <span className={clsx('px-1.5 py-0.5 rounded text-[10px]', isDark ? 'bg-orange-900/40 text-orange-300' : 'bg-orange-100 text-orange-700')}>输出</span>}
                    {projectInfo.structure?.yaml && <span className={clsx('px-1.5 py-0.5 rounded text-[10px]', isDark ? 'bg-cyan-900/40 text-cyan-300' : 'bg-cyan-100 text-cyan-700')}>YAML</span>}
                  </div>
                  <button
                    ...
                    title="在文件管理器中打开项目目录"
                  >
                    <ExternalLink size={10} /> 打开目录
                  </button>
                </>
              )}
            </div>
          ) : (
            <div className={isDark ? 'text-xs text-gray-500' : 'text-xs text-gray-400'}>请先在项目浏览器中选择一个项目</div>
          )}
        </div>

        <div className="space-y-1.5">
          <h4 className={clsx('text-xs font-semibold flex items-center gap-1', isDark ? 'text-gray-200' : 'text-gray-700')}>
            <Settings size={12} /> 参数配置
          </h4>
        ...
        <div className={clsx('border-t pt-3 space-y-1.5', isDark ? 'border-gray-700' : 'border-gray-200')}>
          <h4 className={clsx('text-xs font-semibold', isDark ? 'text-gray-200' : 'text-gray-700')}>批量项目</h4>
          <div className={clsx('flex flex-wrap gap-1 max-h-20 overflow-auto', isDark ? 'text-gray-300' : 'text-gray-700')}>
            {projects.length === 0 ? (
              <span className={isDark ? 'text-[10px] text-gray-500' : 'text-[10px] text-gray-400'}>暂无项目</span>
            ) : (
              projects.map((p) => {
          ...
          <div className="flex flex-col gap-1">
            <Button variant="primary" size="sm" icon={<Play size={12} />} onClick={handleRenderBatch} disabled={isRendering || selectedProjectIds.length === 0} loading={isRendering} className="w-full justify-start">
              批量渲染 ({selectedProjectIds.length})
            </Button>
