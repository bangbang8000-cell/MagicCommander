    ...
    <Modal open={open} title={t('template.edit.title')} onClose={onClose}>
      <div className="space-y-3 text-sm">
        ...
        <input
          ...
          placeholder={t('template.edit.scenarioPlaceholder')}
          className="w-full px-3 py-2 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900"
        />
        <input
          value={sourceProject}
          onChange={(event) => setSourceProject(event.target.value)}
          placeholder={t('template.edit.sourcePlaceholder')}
          className="w-full px-3 py-2 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900"
        />
        <textarea
          value={inputRequirements}
          onChange={(event) => setInputRequirements(event.target.value)}
          placeholder={t('template.edit.inputPlaceholder')}
          className="w-full px-3 py-2 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900"
          rows={3}
        />
        <textarea
          ...
          placeholder={t('template.edit.outputPlaceholder')}
          className="w-full px-3 py-2 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900"
          rows={2}
        />
        <div className="flex justify-end gap-2">
          <button onClick={onClose} className="px-3 py-1.5 rounded border border-gray-200 dark:border-gray-700">
            {t('template.edit.cancel')}
          </button>
          <button
            onClick={submit}
            disabled={loading}
            className="px-3 py-1.5 rounded bg-primary-600 text-white disabled:opacity-50"
          >
