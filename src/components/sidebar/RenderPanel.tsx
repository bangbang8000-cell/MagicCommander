      ...
      <div className="p-3 space-y-3">
                  ...
                  <>
                    ...
                    <div className="flex flex-wrap gap-1 pt-1">
                      ...
                      {projectInfo.structure?.templates && (
                        <span className="px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded text-[10px]">模板</span>
                      )}
                      {projectInfo.structure?.para && (
                        <span className="px-1.5 py-0.5 bg-purple-100 text-purple-700 rounded text-[10px]">参数</span>
                      )}
                      {projectInfo.structure?.output && (
                        <span className="px-1.5 py-0.5 bg-orange-100 text-orange-700 rounded text-[10px]">输出</span>
                      )}
                      {projectInfo.structure?.yaml && (
                        <span className="px-1.5 py-0.5 bg-cyan-100 text-cyan-700 rounded text-[10px]">YAML</span>
                      )}
                    </div>
                  </>
                )}
        ...
        <div className="space-y-1.5">
          <h4 className={clsx('text-xs font-semibold', isDark ? 'text-gray-200' : 'text-gray-700')}>
            选择项目
            <span className={clsx('font-normal ml-1', isDark ? 'text-gray-400' : 'text-gray-500')}>
              ({selectedProjectIds.length}/{projects.length})
            </span>
          </h4>
          <div className={clsx('flex flex-wrap gap-1 max-h-32 overflow-auto rounded p-1', isDark ? 'bg-gray-800' : 'bg-gray-50')}>
            {projects.length === 0 ? (
              <span className={clsx('text-[10px]', isDark ? 'text-gray-400' : 'text-gray-500')}>暂无项目</span>
            ) : (
              projects.map((p) => {
