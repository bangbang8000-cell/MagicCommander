...
export function SettingsPanel() {
  ...
  const ensureAIHubReady = useCallback(async (): Promise<string | null> => {
    try {
      ...
      return (e instanceof Error ? e.message : String(e)) || t('common:settings.ai.hubFailed')
    }
  }, [aiConfig.providers, aiConfig.defaultProvider, t])
  ...
  const handleTestConnection = useCallback(async () => {
    ...
    try {
      ...
      setTestResult({
        ok: false,
        msg: e instanceof Error ? e.message : String(e) || t('common:settings.ai.testFailed'),
      })
    } finally {
      setTesting(false)
    }
  }, [activeProvider, apiKey, model, baseUrl, catalog, ensureAIHubReady, t])
  ...
  const handleFetchModels = useCallback(async () => {
    ...
    try {
      const err = await ensureAIHubReady()
      if (err) {
        console.error('AI Hub not ready:', err)
        return
      }
  ...
  const handleTestPlatformConnection = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPlatformTestResult({
          ok: true,
          msg: `连接成功 - ${data.service || 'MagicCommander Platform'} ${data.version || ''}`,
        })
        setPlatformBaseUrl(platformUrl.trim())
      } else {
        setPlatformTestResult({ ok: false, msg: `服务器返回错误: ${res.status}` })
      }
    } catch (e) {
      setPlatformTestResult({ ok: false, msg: `无法连接: ${e instanceof Error ? e.message : String(e) || '网络错误'}` })
    } finally {
                    ...
                    {Object.entries(LOCALE_NAMES).map(([locale, name]) => (
                      ...
                      >
                        <span className="text-xs font-bold w-5 text-center">
                          {LANGUAGE_ICON_CHARS[locale as keyof typeof LANGUAGE_ICON_CHARS]}
                        </span>
                        <span>{name}</span>
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={clsx(
                  ...
                  generalSettings.autoSave ? 'bg-blue-500' : isDark ? 'bg-gray-600' : 'bg-gray-300',
                )}
              >
                <div
                  className={clsx(
    ...
    <div className="space-y-3">
              ...
              {PROVIDER_KEYS.map((key) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[11px] opacity-70">{isConfigured ? '✓' : '—'}</span>
                  </button>
                )
              })}
            </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-0.5">
                  ...
                  <button
                    ...
                    className={clsx(
                      'text-[11px] flex items-center gap-1 px-1.5 py-0.5 rounded',
                      fetchingModels
            ...
            <div className="flex items-center justify-between mt-3">
              <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  className={clsx(
                    'text-[11px] flex items-center gap-1 px-2 py-1 rounded',
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                ...
                <div
                  className={clsx(
                    ...
                    aiConfig.routingEnabled ? 'bg-blue-500' : isDark ? 'bg-gray-600' : 'bg-gray-300',
                  )}
                >
                  <div
                    className={clsx(
                    ...
                    <div key={taskType} className="flex items-center gap-2">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          const allTypes = ['code', 'analysis', 'simple', 'complex'] as const
                          const merged = allTypes.map(
                            (t) =>
                              newRules.find((r) => r.taskType === t) || {
                                taskType: t,
                        ...
                        className={clsx(
                          ...
                          isDark ? 'bg-gray-700 text-gray-200' : 'bg-gray-100 text-gray-700 border border-gray-300',
                        )}
                      >
                        {PROVIDER_KEYS.filter((k) => aiConfig.providers[k]?.apiKey || k === 'ollama').map((k) => (
                          <option key={k} value={k}>
                            {PROVIDER_CATALOG[k].name}
                          </option>
                        ))}
                        {PROVIDER_KEYS.filter((k) => !aiConfig.providers[k]?.apiKey && k !== 'ollama').map((k) => (
                          <option key={k} value={k} disabled>
                            {PROVIDER_CATALOG[k].name} ({t('common:settings.ai.notConfigured')})
                          </option>
                        ))}
                      </select>
                    </div>
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h4 className={clsx('text-sm font-medium mb-2', isDark ? 'text-gray-200' : 'text-gray-700')}>
    ...
    <div className="space-y-3">
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h4 className={clsx('text-sm font-medium', isDark ? 'text-gray-200' : 'text-gray-700')}>
              {t('common:settings.platform.title') || '平台连接'}
            </h4>
            <p className={clsx('text-xs mt-0.5', isDark ? 'text-gray-500' : 'text-gray-400')}>
            ...
            <div className="mt-3 space-y-2">
              <div>
                <label className={clsx('text-[11px]', isDark ? 'text-gray-500' : 'text-gray-400')}>
                  {t('common:settings.platform.serverUrl') || '服务器地址'}
                </label>
                <div className="flex gap-1.5 mt-0.5">
                  <input
                    ...
                    placeholder="http://evergreenzhou.com"
                    className={clsx(
                      'flex-1 px-2.5 py-1.5 rounded text-xs outline-none',
                      isDark
                        ? 'bg-gray-900 text-gray-100 placeholder-gray-600 focus:ring-1 focus:ring-blue-500'
                        : 'bg-white text-gray-900 placeholder-gray-400 focus:ring-1 focus:ring-blue-400 border border-gray-300',
                    )}
                  ...
                  <button
                    ...
                    disabled={testingPlatform || !platformUrl.trim()}
                    className={clsx(
                      'text-[11px] flex items-center gap-1 px-2.5 py-1.5 rounded whitespace-nowrap',
                      testingPlatform || !platformUrl.trim()
                        ? 'opacity-40 cursor-not-allowed'
                        : isDark
                          ? 'bg-green-900/30 text-green-300 hover:bg-green-900/50'
                          : 'bg-green-100 text-green-700 hover:bg-green-200',
                  ...
                  >
                    {testingPlatform ? <RefreshCw size={10} className="animate-spin" /> : <Play size={10} />}
                    {t('common:settings.platform.testConnection') || '测试连接'}
                  </button>
                </div>
              </div>
              {platformTestResult && (
                ...
                >
                  {platformTestResult.ok ? <Check size={12} /> : <XCircle size={12} />}
                  <span>{platformTestResult.msg}</span>
                </div>
              )}
              <div
                className="border-t pt-2 mt-2"
                style={{ borderColor: isDark ? 'rgb(75, 85, 99)' : 'rgb(229, 231, 235)' }}
              >
                <div className="flex items-center justify-between">
                  <span className={clsx('text-xs', isDark ? 'text-gray-400' : 'text-gray-600')}>
                    {t('common:settings.platform.loginStatus') || '登录状态'}
                  </span>
                  <span
                    className={clsx(
                      'text-xs font-medium',
                      platformLoggedIn ? 'text-green-400' : isDark ? 'text-gray-400' : 'text-gray-500',
                    )}
                  >
                    {platformLoggedIn
                      ? `${t('common:settings.platform.loggedIn') || '已登录'} — ${platformUsername || ''}`
                      : t('common:settings.platform.notLoggedIn') || '未登录'}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="flex-1 min-w-0 space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={clsx(
                  'w-9 h-5 rounded-full transition-colors relative',
                  advancedSettings.debugMode ? 'bg-blue-500' : isDark ? 'bg-gray-600' : 'bg-gray-300',
                )}
              >
                <div
                  className={clsx(
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={clsx(
                  'w-9 h-5 rounded-full transition-colors relative',
                  advancedSettings.aiHubAutoStart ? 'bg-blue-500' : isDark ? 'bg-gray-600' : 'bg-gray-300',
                )}
              >
                <div
                  className={clsx(
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className={clsx('text-xs mt-0.5', isDark ? 'text-gray-500' : 'text-gray-400')}>
              {t('common:settings.about.desc') || '智能网络配置管理工具'}
            </p>
            <div className="mt-3 space-y-2 text-xs">
              <div className="flex justify-between">
                <span className={isDark ? 'text-gray-500' : 'text-gray-400'}>
                  {t('common:settings.about.version') || '版本'}
                </span>
                <span className={isDark ? 'text-gray-300' : 'text-gray-700'}>{buildInfo.displayVersion}</span>
              </div>
              <div className="flex justify-between">
