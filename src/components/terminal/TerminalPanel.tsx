...
╚══════════════════════════════════════════════════════════════╝`,
  ...
  const ctx: CommandContext = {
    ...
    clearTerminal: () => {
      setHistory([{ input: '', output: '终端已清空。', level: 'info' }])
    },
    selectProject: (name) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (hasAsync) {
        appendOutput('info', '执行中...')
      }
      await executeCommand(trimmed, ctx)
    } catch (err) {
      appendOutput('error', `执行错误: ${(err as Error).message}`)
    } finally {
    ...
    <div className="flex flex-col h-full">
      ...
      >
        {history.map((entry, idx) => (
          <div key={idx} className="flex flex-col">
            ...
            {entry.output && (
              <div className={clsx('px-1 py-0.5 pl-4 break-all', levelColor[entry.level])}>
                {entry.output}
              </div>
            )}
          </div>
      ...
      >
        ...
        <input
          ...
          autoCapitalize="off"
          autoComplete="off"
          placeholder={isExecuting ? '执行中...' : '输入命令，Enter 提交'}
          className={clsx(
